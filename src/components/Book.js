...
const Book = ({book,editID,Changeid,SetAllbooks}) => {
    const [editName,SeteidtName] = useState(book.book_name);
    ...
    const editbook=async (book_id)=>{
        ...
        try{
          ...
          if(result && result.success){
            console.log("success");
            Changeid('');
            SetAllbooks((prev)=> [...prev.filter(t=> t.book_id!==book_id),
                {
                    book_id:book_id,
                    book_name:editName,
                    author:editAuthor,
            ...
            runInAction(()=>{
            BookStore.Allbooks=[...BookStore.Allbooks.filter(t=> t.book_id!==book_id),
                {
                    book_id:book_id,
                    book_name:editName,
                    author:editAuthor,
    ...
    if(editID!==book.book_id){
        return ( 
            <tr>
                ...
                <td>{book.yop}</td>
                <td><i className="fa fa-pencil-square-o" aria-hidden="true" onClick={()=> Changeid(book.book_id)}></i></td>
            </tr>
        );
    }else{
        return (
            <tr>
                ...
                <td>
                    <i className="fa fa-floppy-o" aria-hidden="true" onClick={()=> editbook(book.book_id)}></i>
                </td>
            </tr>
        );
    }
